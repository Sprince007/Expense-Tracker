        ...
        <Link href={'/dashboard/expenses/' + budget?.id}>
            <div className='p-5 border rounded-lg hover:shadow-md cursor-pointer h-[170px]'>
                ...
                <div className='flex gap-2 items-center justify-between'>
                    ...
                    <h2 className='font-bold text-purple-600 text-lg'>{formatAmount(budget.amount)} FCFA</h2>
                </div>
                <div className='mt-5'>
                    <div className='flex items-center justify-between mb-3'>
                        <h2 className='text-xs text-slate-400'>{formatAmount(budget.totalSpent ? budget.totalSpent : 0)} FCFA Spent</h2>
                        <h2 className='text-xs text-slate-400'>{budget.from}</h2>
                        <h2 className='text-xs text-slate-400'>{formatAmount(budget.amount - budget.totalSpent)} FCFA Remaining</h2>
                    </div>
